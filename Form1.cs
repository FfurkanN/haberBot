...
namespace haberBot
{
    public partial class Form1 : Form
    {
        ...
        private async void GetNewsDocument(string collection)
        {
            ...
            QuerySnapshot snapshots = await newsCollection.GetSnapshotAsync();

            var sortedDocuments = snapshots.Documents
                .OrderByDescending(doc => DateTime.ParseExact(doc.GetValue<string>("newsDate"), "dd.MM.yyyy", CultureInfo.InvariantCulture))
                .ToList();
            int y = 5;
            foreach (DocumentSnapshot documentSnapshot in sortedDocuments)
            {
        ...
        private void techInsideNews()
        {
            ...
            while (true)
            {
                ...
                if (newsDate.Equals(dateNow) || newsDate.Equals((DateTime.Now.AddDays(-1)).ToString("dd.MM.yyyy")) || newsDate.Equals((DateTime.Now.AddDays(-2)).ToString("dd.MM.yyyy")))
                {
                    script = @"
        ...
        private void shiftdeleteNews()
        {
            ...
            while(true)
            {
                ...
                newsDate = ParseTime(newsDate);
                DateTime currentTime = DateTime.Now;
                string dateNow = currentTime.ToString("dd.MM.yyyy");
        ...
        private void mashableNews()
        {
            ...
            for(int i=1;i<=3;i++)
            {
                ...
                string newsDate = ParseTime(datetimeValue.Trim());
        ...
        static string ParseTime(string timeText)
        {
            ...
            else if (DateTime.TryParseExact(timeText, new[] { "MMMM dd, yyyy", "MMMM d, yyyy"}, new CultureInfo("en-US"), DateTimeStyles.None, out DateTime date3))
            {
                return date3.ToString("dd.MM.yyyy");
            }
            else if (DateTime.TryParseExact(timeText.Trim(), new[] { "dd MMMM, yyyy", "d MMMM, yyyy" }, new CultureInfo("en-US"), DateTimeStyles.None, out DateTime date4))
            {
                return date4.ToString("dd.MM.yyyy");
            }
            else
            {
